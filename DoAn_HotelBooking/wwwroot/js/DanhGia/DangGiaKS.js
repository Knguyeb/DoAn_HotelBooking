$(document).ready(function () {
    ...
    $(document).on("click", "#btnDanhGia", function (e) {
        e.preventDefault();
        const maKhachSan = $(this).data("makhachsan");

        if (!maKhachSan) {
            console.error("Không tìm thấy mã khách sạn (data-makhachsan)!");
            return;
        }

        // 🌟 KIỂM TRA TRƯỚC KHI MỞ FORM
        $.ajax({
            url: '/DanhGiaKSs/KiemTraDanhGia',
            type: 'GET',
            data: { maKhachSan: maKhachSan },
            success: function (checkRes) {
                // Nếu chưa đăng nhập hoặc ĐÃ ĐÁNH GIÁ RỒI -> Quăng thông báo chặn luôn
                if (!checkRes.hopLe) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Thông báo',
                ...
                // 🌟 NẾU CHƯA ĐÁNH GIÁ -> MỞ FORM 5 SAO BÌNH THƯỜNG
                Swal.fire({
                    title: "Chọn số sao đánh giá",
                    background: '#242526',
                    color: '#fff',
                    html: `
                        <form class="rating" id="starRatingForm">
                            ${[5, 4, 3, 2, 1].map(i => `
                                <input class="rating__input" type="radio" id="ratingKS-${i}" name="rating" value="${i}">
                                <label class="rating__label" for="ratingKS-${i}" aria-label="${i} sao">
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function (res) {
                                // 🌟 SỬA TẠI ĐÂY: Bắt cả errorMessage nếu có lỗi từ Backend
